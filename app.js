...
let enviarSiguiente = document.querySelector('#btn-send')
let alerta = document.querySelector('#alerta')
let sonidoEnviado = document.querySelector('#send-sound')
let sonidoReintentar = document.querySelector('#tryAgain-sound');
let sonidoFallido = document.querySelector('#failed-sound');
...
function enviar() {
    ...
    if (!colaDeCorreos.first) {
        alerta.classList.remove('d-none') // ahora se ve
        setTimeout(() => {
            alerta.classList.add('d-none') // ahora ya no :v
        }, 2000)
        return //lo detiene, no lo deja pasar de aqui
    }

    alerta.classList.add('d-none') //paso la prueba, entonces si hay correos.

    if (banderita) {
        return // si ya hay un correo enviandose, no hace absolutamente nada
    }
    ...
    let correoEnviado = colaDeCorreos.porEnviar(correoAPuntoDeEnviarse); // lo saco de la cola
    ...
    if (correoEnviado) {
        ...
        if (correoEnviado.estado == 'Envio exitoso') {
         estadoSetTimeOut = `<div class="p-2 alert alert-success m-0 text-center">${correoEnviado.estado}</div>`;
            ...
            estadoSetTimeOut = `<div class="p-2 alert alert-danger m-0 text-center">${correoEnviado.estado}</div>`;
        }

        renderizar(); // renderizo para actualizar la lista visual sin el correo que salió
        ...
        setTimeout(() => {
            contenedorCorreosActuales.innerHTML = `
                <div id="current-mail" class="mail-card-placeholder p-3 text-center">
...
enviarSiguiente.addEventListener('click', enviar)
